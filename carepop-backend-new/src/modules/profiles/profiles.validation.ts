...
export const updateProfileSchema = z.object({
  ...
  date_of_birth: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'DOB must be in YYYY-MM-DD format'),
  contact_no: z.string().optional().nullable(),
  gender_identity: z.string().min(1).optional(),
  pronouns: z.string().min(1).optional(),
  assigned_sex_at_birth: z.string().min(1).optional(),
  ...
  city_municipality_code: z.string().min(1),
