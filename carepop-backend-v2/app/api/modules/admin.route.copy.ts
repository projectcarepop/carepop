    ...
    try {
      const deletedItems = await db.transaction(async (tx) => {
        const [currentItem] = await tx.select({
            ...
            quantityOnHand: inventory_items.quantityOnHand,
            clinicId: inventory_items.clinicId
...
adminRoutes.get('/users', zValidator('query', adminUsersQuerySchema), async (c) => {
    ...
    try {
        ...
        if (error) {
            console.error('[ADMIN USERS] Supabase listUsers error:', error);
            throw new Error(error.message || 'Failed to list users from Supabase Auth.');
        }

        let users = usersResponse.users;
        console.log(`[ADMIN USERS] Fetched ${users.length} users from Supabase Auth.`);

        const userIds = users.map(u => u.id);
        if (userIds.length === 0) {
            return c.json({
                data: [],
                pagination: { page, pageSize: limit, totalCount: 0, totalPages: 0 }
            });
        }
        
        const profilesQuery = db.select().from(profiles).where(inArray(profiles.id, userIds));
        const profilesData = await profilesQuery;
        console.log(`[ADMIN USERS] Fetched ${profilesData.length} profiles from DB.`);
        ...
        if (role) {
            console.log(`[ADMIN USERS] Filtering by role: ${role}`);
            filteredData = combinedData.filter(u => u.role === role);
        }
        if (q) {
            console.log(`[ADMIN USERS] Filtering by query: ${q}`);
            const lowerCaseQuery = q.toLowerCase();
            filteredData = filteredData.filter(u =>
                (u.email && u.email.toLowerCase().includes(lowerCaseQuery)) ||
                (u.firstName && u.firstName.toLowerCase().includes(lowerCaseQuery)) ||
                (u.lastName && u.lastName.toLowerCase().includes(lowerCaseQuery))
            );
        }
        
        console.log(`[ADMIN USERS] Final filtered data count: ${filteredData.length}`);

        const totalCount = usersResponse.users.length < limit ? (page - 1) * limit + usersResponse.users.length : page * limit + 1;
