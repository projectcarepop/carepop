...
ALTER TABLE "profiles" ADD CONSTRAINT "fk_profiles_id" FOREIGN KEY ("id") REFERENCES "auth"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE POLICY "Anyone can view public data." ON "services" AS PERMISSIVE FOR SELECT TO public;--> statement-breakpoint
...
CREATE POLICY "Patients can view their own order items." ON "patient_order_items" AS PERMISSIVE FOR SELECT TO public;
*/