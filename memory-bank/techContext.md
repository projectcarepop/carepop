...
### A. Native Mobile Application (\`carepop-nativeapp/\` - iOS & Android ONLY)  
    ...
    *   Primary Navigation Orchestration: `carepop-mobile/App.tsx` currently serves as the main entry point and orchestrates top-level navigation, including conditional logic for onboarding, authentication, profile creation, and the main application drawer. The role and usage of `carepop-mobile/src/navigation/RootNavigator.tsx` should be reviewed for potential refactoring or deprecation to ensure a single source of truth for root navigation.
...
*   Primary BaaS: Supabase  
    ...
    *   Triggers/Functions: Supabase Database Functions (PL/pgSQL) and Triggers used for automated database operations (e.g., \`handle_new_user\` profile creation). Supabase Edge Functions (Deno) might be used for very simple, data-proximate HTTP endpoints if needed later.  
    *   Storage: Supabase Storage for file uploads, secured by Storage policies.  
    *   SDK: Supabase JS SDK (\`@supabase/supabase-js\`) used by clients and Cloud Run services. Separate \`anon\` (public/client-safe) and \`service_role\` (privileged, bypasses RLS, used ONLY within secure Cloud Run functions after explicit authorization checks) client instances initialized (\`carepop-backend/src/config/supabaseClient.ts\`).
